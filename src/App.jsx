...
const appId = "autoparts-b4a5c"; // ID de tu proyecto
...
const App = () => {
  ...
  useEffect(() => {
    const initAuth = async () => {
      ...
      } catch (error) { setDbError("Reconectando con el servidor..."); }
    };
    initAuth();
  ...
  // Control de Vistas (Navegación por Hash)
  ...
  useEffect(() => {
    ...
    // Contar visita solo si es la carga inicial
    ...
    const unsubProds = onSnapshot(productsCol, (snap) => {
      ...
      setDbError(null);
    }, () => setDbError("Sincronizando con la nube..."));
  ...
  // Funciones de Carrito e Interacción
  const addToCart = (p) => {
    if (!p) return;
    if (cart.length === 0) {
      const statsRef = doc(db, 'artifacts', appId, 'public', 'data', 'stats', 'global');
      ...
      <main className="relative z-10">
        ...
        {view === 'admin-login' && <AdminLogin onLogin={(u, p) => { 
          if (u === 'admin' && p === 'AutoPrecision2024*') { 
            ...
            // El useEffect de navegación hará el cambio de vista automáticamente
          } else {
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 backdrop-blur-md bg-gray-950/40 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-3xl overflow-hidden animate-in zoom-in duration-300">
            ...
            <div className="p-8 md:p-12 space-y-6 text-sm text-gray-600">
              <p>En <strong>autoparts.lat</strong> usamos cookies técnicas para recordar tu pedido. Tus búsquedas nos ayudan a mejorar el inventario en Valencia.</p>
              <p>No compartimos tus datos con terceros. La venta se concreta de forma segura mediante asesoría humana en WhatsApp.</p>
            </div>
...
const AdminLogin = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-[75vh] flex items-center justify-center p-6 animate-in fade-in zoom-in duration-500 text-gray-950">
      <div className="bg-white p-10 md:p-14 rounded-[3.5rem] shadow-2xl border border-indigo-50 w-full max-w-md text-center">
        <div className="bg-indigo-600 w-20 h-20 rounded-[2rem] flex items-center justify-center mb-10 mx-auto text-white shadow-xl shadow-indigo-100"><Lock size={40} /></div>
        <h2 className="text-3xl font-black mb-10 uppercase italic leading-none">Admin Login</h2>
        <form onSubmit={(e) => { e.preventDefault(); onLogin(u, p); }} className="space-y-5 text-left">
          <input type="text" placeholder="Usuario" className="w-full bg-[#f8faff] border-none rounded-2xl py-5 px-8 focus:ring-4 ring-indigo-50 outline-none font-bold" value={u} onChange={(e) => setU(e.target.value)} />
          <input type="password" placeholder="Clave" className="w-full bg-[#f8faff] border-none rounded-2xl py-5 px-8 focus:ring-4 ring-indigo-50 outline-none font-bold" value={p} onChange={(e) => setP(e.target.value)} />
          <button className="w-full bg-gray-950 text-white py-6 rounded-[2rem] font-black text-xl hover:bg-indigo-600 transition-all shadow-xl mt-4">Acceder</button>
        </form>
      </div>
    </div>
  );
    ...
    <div className="p-6 md:p-12 max-w-7xl mx-auto py-12 animate-in fade-in slide-in-from-bottom-8 duration-700 text-gray-950 text-left">
        ...
        <div className="mb-12 bg-white border-4 border-dashed border-indigo-100 rounded-[3rem] p-8 md:p-12 animate-in zoom-in duration-300 shadow-sm text-center">
           {!importing ? (
             <div>
               ...
               <label className="bg-gray-950 text-white px-10 py-5 rounded-2xl font-black text-sm uppercase tracking-widest cursor-pointer hover:bg-indigo-600 transition-all inline-block mt-4">Seleccionar Archivo Excel<input type="file" className="hidden" accept=".xlsx, .xls, .csv" onChange={handleExcel} /></label>
             </div>
           ) : (
             <div className="py-10">
               <Loader2 className="w-16 h-16 text-indigo-600 animate-spin mx-auto mb-6" />
               <p className="font-black text-indigo-600">{importProgress}% Completado</p>
             </div>
           )}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
        {[ {icon: TrendingUp, label: 'Visitas', val: stats.totalVisits},
           ...
           {icon: Target, label: 'Consultas', val: stats.totalOrdersClicked},
           {icon: BarChart2, label: 'Conversión', val: `${convRate}%`}
        ].map((m, i) => (
          <div key={i} className="bg-white p-8 rounded-[2.5rem] border border-indigo-50/50 shadow-sm"><div className="flex items-center gap-3 text-indigo-600 mb-2 font-black uppercase tracking-widest text-[10px]"><m.icon size={18}/><span>{m.label}</span></div><p className="text-4xl font-black">{m.val || 0}</p></div>
        ))}
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-10 rounded-[3.5rem] border border-indigo-50 shadow-sm text-left">
          <div className="flex items-center gap-3 mb-10"><Target className="text-emerald-500" size={24}/><h3 className="text-2xl font-black italic uppercase leading-none text-left">Top Buscados</h3></div>
          <div className="space-y-4 text-left">
            {[...products].sort((a,b) => (b.searches||0)-(a.searches||0)).slice(0, 5).map((p, idx) => (
              <div key={p.code + idx} className="flex items-center justify-between p-6 bg-[#f8faff] rounded-[2rem] border border-transparent hover:border-indigo-100 transition-all text-left">
                <div className="flex items-center gap-6"><span className="font-black text-gray-400">{idx+1}</span><div><p className="font-bold text-lg">{p.name}</p><p className="text-[10px] text-gray-400 font-black uppercase">{p.brand}</p></div></div>
        ...
        <div className="bg-white p-10 rounded-[3.5rem] border border-indigo-50 shadow-sm text-left">
          <div className="flex items-center gap-3 mb-10"><Eye className="text-indigo-600" size={24}/><h3 className="text-2xl font-black italic uppercase leading-none">Radar de Búsquedas</h3></div>
          <div className="space-y-3">
            {searchLogs.map((log) => (
              <div key={log.id} className="flex items-center justify-between p-4 bg-[#f8faff] rounded-2xl border border-indigo-50/50">
                <div className="flex items-center gap-3 font-bold text-gray-700"><History size={14} className="text-indigo-300" /> "{log.term}"</div>
...
// Vistas secundarias compactas para asegurar rendimiento
const LandingView = ({ searchTerm, setSearchTerm, onSearch }) => (
  <div className="min-h-[80vh] flex flex-col items-center justify-center px-6 text-center animate-in fade-in duration-1000">
    <div className="max-w-4xl w-full">
      <div className="inline-flex items-center gap-2 bg-indigo-50 text-indigo-600 px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-widest mb-10 shadow-sm"><Zap size={14} className="animate-pulse" /> Expertos en Valencia</div>
      <h2 className="text-5xl md:text-8xl font-black tracking-tighter mb-8 leading-none">El repuesto <br /> <span className="text-indigo-600 italic">que buscas.</span></h2>
      <p className="text-gray-400 font-medium text-lg mb-14 max-w-2xl mx-auto">Venta especializada de repuestos automotrices multimarca. Tren delantero, motor y más.</p>
      <form onSubmit={onSearch} className="relative group max-w-2xl mx-auto px-2">
        <div className="relative bg-white p-3 md:p-5 rounded-[2.2rem] md:rounded-[3rem] shadow-2xl border border-indigo-100/30 flex items-center px-6 md:px-12">
          <Search className="text-indigo-300 w-7 h-7" />
          <input type="text" autoFocus placeholder="Ej: Amortiguador Hilux" className="w-full py-4 md:py-6 px-4 md:px-8 border-none bg-transparent focus:ring-0 outline-none text-xl md:text-3xl font-bold text-gray-950 placeholder:text-gray-200" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
...
const CatalogListView = ({ products, onAddToCart, onGoBack, searchTerm, setSearchTerm }) => (
  <div className="pb-32 max-w-6xl mx-auto px-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
    <div className="mt-8 mb-10 flex items-center justify-between gap-6">
      <div className="bg-white p-1 rounded-[2.5rem] flex items-center px-6 flex-1 border border-indigo-50 shadow-sm">
      ...
      {products.map((p, idx) => (
        <div key={p.code || idx} className="flex flex-col md:grid md:grid-cols-12 gap-5 items-center px-7 py-7 md:px-12 md:py-10 bg-white border border-indigo-50/50 rounded-[3rem] hover:border-indigo-100 hover:shadow-xl transition-all group text-left">
          <div className="col-span-1 md:col-span-6 w-full">
            ...
            <div className="flex gap-3"><span className="text-[10px] font-black bg-indigo-50 text-indigo-600 px-4 py-2 rounded-xl uppercase tracking-widest">{p.category || 'Repuestos'}</span><span className="text-[10px] font-bold text-gray-400 border border-gray-100 px-4 py-2 rounded-xl uppercase tracking-widest">{p.model}</span></div>
          </div>
          <div className="col-span-1 md:col-span-3 w-full flex items-center justify-between md:flex-col gap-3">
            <code className="text-[11px] bg-[#f8faff] px-4 py-2 rounded-xl font-mono text-gray-700 border border-indigo-50/50">{p.code || 'S/N'}</code>
            <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">{p.brand || 'Genérico'}</p>
          </div>
...
const CartView = ({ cart, updateQty, removeFromCart, onConfirm }) => (
  <div className="p-6 md:p-10 max-w-4xl mx-auto py-12 md:py-24 animate-in fade-in slide-in-from-bottom-12 duration-700 text-gray-900 text-left">
               ...
               <div className="flex-1">
                 ...
                 <div className="flex items-center gap-8 mt-6">
                   ...
                   <div className="flex items-center bg-gray-100 rounded-3xl px-4 py-2 border">
                     <button onClick={() => updateQty(item.id, -1)} className="p-2 text-gray-400 hover:text-indigo-600 transition-all"><Minus size={18}/></button>
                     <span className="w-12 text-center font-black text-xl">{item.qty}</span>
                     <button onClick={() => updateQty(item.id, 1)} className="p-2 text-gray-400 hover:text-indigo-600 transition-all"><Plus size={18}/></button>
                   </div>
                 </div>
               </div>
               <button onClick={() => removeFromCart(item.id)} className="w-full md:w-auto p-5 text-red-500 hover:text-red-700 transition-all flex items-center justify-center"><Trash2 size={24} /></button>
            </div>
         ))}
         <div className="mt-14 bg-gray-950 p-10 rounded-[4rem] text-center shadow-3xl relative overflow-hidden text-white">
  ...
  <footer className="mt-20 border-t border-gray-100 py-16 px-6 bg-white/50 text-gray-950 text-center">
    <div className="max-w-6xl mx-auto flex flex-col items-center gap-8">
      <div className="flex items-center gap-8">
        ...
        <div className="flex items-center gap-2 text-gray-400"><MapPin size={16} /><p className="text-[11px] font-bold uppercase tracking-widest">Valencia, Venezuela</p></div>
      </div>
